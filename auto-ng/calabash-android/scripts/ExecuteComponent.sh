...
function SendEmailReport {
  ...
  echo -e "mail -s \"*** Android Sanity Test Result  $buildNumber, Device: $device\"  mhadimani@ipass.com -- -f Automation_Report@ipass.com </adrdata/adr/prod/Platforms/Handheld/OMA/$projectNumber/Reports/$reportsFileName.log ***">>"/opt/auto-ng/calabash-android/Main/Project/$reportsFileName.mail";
...
then
	testcases='features/Sanity.feature';
	echo "Sanity Test"
elif [ "$component" == "GUI" ];
then
...
then
	testcases='features/Connectivity.feature';
...
then
	testcases='features/Activation.feature';
...
then
	...
	then
		...
		pwd
		eval "calabash-android run WiFiMobilize.apk $testcases -p wfm -f json -o /opt/auto-ng/calabash-android/Main/Project/cucumber.json -f html -o /opt/auto-ng/calabash-android/Main/Project/cucumber.html"
	...
	then
		...
		cd /opt/auto-ng/calabash-android/Main/Project/
		pwd
		eval "calabash-android run OpenMobileAndroidClient.apk $testcases -p android -f json -o ./cucumber.json -f html -o /opt/auto-ng/calabash-android/Main/Project/cucumber.html"
	fi
fi
...
echo "ziping reports"
zip "$component"."zip" *screenshot* cucumber.json cucumber.html ReportData.txt log.txt
rm *screenshot*
rm result.txt
rm cucumber.json #Added by Mahesh
rm ReportData.txt #Added by Mahesh
rm cucumber.html #Added by Mahesh
rm log.txt #Added by Mahesh
